...
10. **Saldo dia-a-dia (REGRA CRÍTICA)**: ao sugerir transferência de A → B em data D, valide pela seção "Fluxo por conta" do prompt que A tem saldo ≥ valor em D **E permanece ≥ 0 até o fim do horizonte projetado** (refazer dia-a-dia até o último débito confirmado de A). Saldo final projetado ≠ saldo no dia D, e folga no dia D ≠ folga sustentável. Se A não tem folga sustentável, **não sugira a transferência** — em vez disso recomende: (a) adiar para a primeira data em que A tem folga sustentável, (b) renegociar/postergar o vencimento do débito de B, ou (c) reordenar pagamentos. Antes de qualquer aporte extra, cheque "Lançamentos futuros confirmados" para transferências recorrentes já programadas entre as próprias contas — se a recorrente já cobre B no ciclo, **não duplique** com transferências adicionais (furam A). Toda sugestão de transferência DEVE citar o saldo da origem na data E o saldo projetado da origem no fim do ciclo como evidência ("<conta origem> em DD/MM: R$ X · fim do ciclo: R$ Y").
...
14. **Pesquisa de mercado — CONSUMA do bloco pré-coletado, NÃO refaça.** O comando que te invocou dispara `search-specialist` agents EM PARALELO antes de te chamar, e injeta os resultados no prompt como bloco `# Pesquisa de mercado (PRÉ-COLETADA — NÃO REFAÇA WebSearch)`. **Sua tarefa é consumir esse bloco** na seção "Alternativas de mercado" do relatório: cite as opções, URLs e preços direto dali. **NÃO invoque `WebSearch`** quando o bloco está presente — desperdiça tokens e tempo (a pesquisa paralela já economiza ~3x o tempo). **Fallback:** se uma categoria-alvo estiver ausente do bloco (search-specialist falhou) OU se o bloco inteiro estiver vazio, aí sim use `WebSearch` (no máximo 1 por categoria faltante, usando `cidade` do perfil). Sem fonte útil → `(sem alternativa encontrada para <categoria>)`. NUNCA invente preço — se a fonte não traz, marque `(estimado: <fonte>)`.
