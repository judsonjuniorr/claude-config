...
| **herow-dev** | Commands: `/herow-dev:blueprint·quick·execute` (gstack plan→worktree→ship flow, graphify-integrated), `/herow-dev:code:review·refactor·generate-tests` (code:review runs multi-agent parallel review with language-aware dispatch — `.tsx`/`.ts`/`.py` trigger specialist reviewers — plus a second-opinion pass that CONFIRM/DISPUTE/ESCALATEs findings), `/herow-dev:git:pr·fix-conflicts·release-notes`, `/herow-dev:react:review·test·validate-ui`, `/herow-dev:python:review·fastapi-review`. Skills: error-handling, prompt-optimizer, deep-research, exa-search, jira-integration. 17 specialist agents (fullstack, python-pro, reviewers, tdd-guide, ui-ux, …) |
