...
> **Optional review:** before the `--message` call or `pr.sh create`, you may delegate the staged diff to `herow-core:code-reviewer` (Agent tool) for a security/correctness/performance pass. Skip it when that agent isn't in the session's agent list, or when the diff is trivial (typo, docstring, formatting).
