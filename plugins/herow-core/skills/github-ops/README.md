...
└── hooks/
    ├── git-guard.sh      # PreToolUse/Bash — auto-allow read-only + non-destructive writes, ask on destructive writes
    └── tests/
...
- **`git-guard`** (`PreToolUse`/`Bash`) — three-tier: read-only `gh`/`glab` calls **and** non-destructive writes are **auto-allowed outright, no prompt**. Read-only, `gh`: `pr view/list/diff/checks/status/checkout/reviews`, `issue view/list/status`, `release view/list/download`, `run view/list/watch/download`, `workflow view/list`, `repo view/list`, `label/cache/variable/secret/gist/extension list`, `browse`, `status`, `auth status`, `--version`, `search`. Read-only, `glab`: `mr view/list/diff/checkout/checks`, `issue view/list`, `release view/list`, `ci view/list/status/trace`, `pipeline list`, `repo view`, `auth status`. Non-destructive writes, `gh`: `pr ready/comment/review/reopen/lock/unlock/update-branch`, `issue comment/reopen/pin/unpin/transfer/lock/unlock/develop`, `release upload`, `run rerun`, `workflow run/enable/disable`. Non-destructive writes, `glab`: `mr note/approve/revoke/rebase/todo/subscribe/unsubscribe`, `issue note/reopen/subscribe/unsubscribe`, `release upload`, `ci retry/run/trigger`. Plus read-only `gh api`/`glab api` GETs, and allowed invocations of the skill's own scripts — `inspect.sh`, `commit-msg.sh`, `pr.sh view|list|checks|diff|ready`, `issue.sh view|list|comment`, `repo.sh info|releases|runs|workflow-run`. The destructive verbs (`create`, `edit`, `close`, `delete`, `delete-asset`, `cancel`, `merge`, glab's `update`) surface an `ask` nudging toward the matching script, but only on the families this hook classifies (`gh pr`/`issue`/`release`/`run`/`workflow` and `glab mr`/`issue`/`ci`/`release`) **and** only when that destructive verb is actually present in the command — a read-only or non-destructive-write command that merely fails the fast-allow (piped to an unrecognized helper like `python3`/`bat`, or a `;`/`|`/`&` inside a quoted argument) gets no decision from this hook at all, not an `ask` (unless its text itself contains a destructive `gh`/`glab` phrase — the destructive match is textual, so that asks): it's silent, and normal Bash permission rules decide (e.g. the `Bash(gh pr list *)` allow rules in settings.json). A destructive verb on any other gh/glab family (`gh secret delete`, `gh label create`, …) also gets no decision from this hook. Raw `git commit`/`git push` prompt as usual too. Read-only `git status`/`diff`/`log` are left to RTK's proxy (no overlap); an `rtk proxy`/`rtk` prefix is stripped before matching any tier. Prefer `jq` (already in the safe-helper list) over `python3 -c` for post-processing `--json` output — it stays inside the fast-allow and never needs a decision at all. Independently of every tier, the hook hard-blocks any Claude Code/Anthropic attribution reference (`Co-Authored-By: Claude`, `Generated with Claude Code`, `Claude-Session:`, `claude.ai/code`) from landing in a commit or PR/MR message, scoped to commands that actually carry one. It inspects the command text only: a message passed by file (`git commit -F`, `--body-file`) is not read. Only `pr.sh` scrubs a body file (`strip_attribution`); `issue.sh` and raw `-F`/`--body-file` calls pass it through unchecked.
