...
- `model: opusplan` — Opus for planning, auto-switches to Sonnet for execution; 1M context in plan phase
- `advisorModel: opus` — advisor review step uses Opus for quality
- `effortLevel: high` — high reasoning effort by default
- `autoCompact: true` — auto-compact at 80% of context window (160K on Sonnet, 800K on Opus)
