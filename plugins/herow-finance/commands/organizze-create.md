...
Rode o script **sem** `--apply` com as flags resolvidas:
```bash
python3 "$SCRIPT" --conta "<conta>" --despesa --valor <v> --data <YYYY-MM-DD> --categoria "<cat>" "<descrição>"
```
Leia o `info|resolve|...`, `info|category|...`, `info|dry-run|...` e `info|payload|...`. Renderize ao usuário um **resumo humano** do alvo resolvido por extenso (ex.: "Nubank → fatura de julho", não "card 3 invoice 189") + valor + categoria. **Não** despeje JSON cru por padrão (ofereça o payload mascarado só se pedirem).
