...
- **`gsc-export-path`** — a path to a Google Search Console export, in one of two shapes:
  ...
  - **Page Indexing / Coverage export** (for `/herow-seo:indexation-check`) — read the `Reason` / `Coverage state` columns.
...
`/herow-seo:cost-guard` is policy-only — it doesn't read GSC itself; it sets the model tier the data commands use to parse the export.
...
├── geo-optimize.md      # /herow-seo:geo-optimize
├── catalog-pages.md     # /herow-seo:catalog-pages
├── ctr-tune.md          # /herow-seo:ctr-tune
├── conversion-track.md  # /herow-seo:conversion-track
├── backlink-outreach.md # /herow-seo:backlink-outreach
├── cost-guard.md        # /herow-seo:cost-guard
├── report.md            # /herow-seo:report
└── launch.md            # /herow-seo:launch (heavy orchestrator)
```
