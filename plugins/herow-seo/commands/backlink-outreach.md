...
2. **Find realistic targets.** Delegate to the **`herow-seo:seo-strategist`** agent via the `Agent` tool (fall back to `general-purpose` if the agent isn't in the session's agent list — same precedent as the rest of the suite) to find relevant, *realistic* outreach targets via `WebSearch` (relevant blogs, resource pages, communities where the niche actually gathers). **Never** spammy mass lists — a short list of genuine fits beats a thousand cold domains.
